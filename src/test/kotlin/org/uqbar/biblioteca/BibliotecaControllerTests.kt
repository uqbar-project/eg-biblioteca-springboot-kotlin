...
class BibliotecaControllerTest(@Autowired val mockMvc: MockMvc) {
   ...
   fun `Se puede obtener un libro a partir de su ID`() {
      mockMvc.perform(MockMvcRequestBuilders.get("/libros/5"))
         .andExpect(MockMvcResultMatchers.status().isOk)
         .andExpect(MockMvcResultMatchers.content().contentType("application/json"))
         .andExpect(MockMvcResultMatchers.jsonPath("$.titulo").value("Ficciones"))
   }
   ...
   fun `Se pueden eliminar libros a partir de su ID`() {
      mockMvc.perform(MockMvcRequestBuilders.delete("/libros/13"))
         .andExpect(MockMvcResultMatchers.status().isOk)
